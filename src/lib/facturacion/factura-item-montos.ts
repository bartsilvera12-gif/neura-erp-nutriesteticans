...
/**
 * Alias legacy — heraclio's `LineaVenta.tipo_iva` usa "EXENTA"|"5%"|"10%".
 * Acepta ambos vocabularios (viejo y nuevo) para no romper `crear-factura-desde-venta.ts`.
 * Default 10 (mismo criterio que `tasaIvaDesdeIvaTipo`).
 */
export function tasaIvaDesdeTipoIvaVenta(v: unknown): TasaIvaItem {
  const s = typeof v === "string" ? v.trim().toUpperCase() : "";
  if (s === "EXENTA") return 0;
  if (s === "5%" || s === "IVA_5") return 5;
  if (s === "10%" || s === "IVA_10") return 10;
  // Fallback al nuevo vocabulario ("exenta"|"iva_5"|"iva_10").
  return tasaIvaDesdeIvaTipo(v);
}
