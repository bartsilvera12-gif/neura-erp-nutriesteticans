...
export function pathRequiresModuleSlug(pathname: string): string | null {
  ...
  // Heraclio: la nota de remisión vive como sub-item bajo "Ventas" en el sidebar
  // (no hay módulo `remision` propio en empresa_modulos). El gate se hace por
  // el módulo `ventas`, así el operador entra por la misma tarjeta que la factura.
