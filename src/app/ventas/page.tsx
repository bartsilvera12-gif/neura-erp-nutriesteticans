            ...
            <tbody>
              {filtradas.length === 0 ? (
                ...
                filtradas.map((v) => {
                  ...
                  return (
                    <tr key={v.id} className={`border-b border-slate-200 last:border-0 hover:bg-[#4FAEB2]/[0.04] transition-colors ${anulada ? "opacity-60" : ""}`} title={anulada ? `Anulada${v.anulacion_motivo ? ` — ${v.anulacion_motivo}` : ""}` : undefined}>
