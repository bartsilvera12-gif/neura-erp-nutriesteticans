...
function KpiCard({
    ...
    return (
      ...
      >
        <div className="flex items-start justify-between gap-2">
          {icon ? <div className="text-2xl opacity-90">{icon}</div> : <div />}
          {variation !== undefined && (
            <span
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        {icon ? <div className="text-2xl">{icon}</div> : <div />}
        {variation !== undefined && (
          <span
            className={`inline-flex items-center gap-0.5 rounded-full px-2 py-0.5 text-xs font-semibold ${
...
function DashInventario({
  ...
  return (
    <div className="space-y-5">
      ...
      {/* KPIs */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Productos totales"      value={String(totalProductos)} color="text-[#0EA5E9]" variation={4} />
        <KpiCard label="Stock total (unidades)" value={formatGs(totalUnidades)} color="text-[#0EA5E9]" />
        ...
        <KpiCard label="Valor del inventario"   value={`Gs. ${formatGsFull(valorTotal)}`} color="text-[#0EA5E9]" variation={12} />
      </div>
...
export default function DashboardPage() {
  ...
  // Nutriestéticans: dashboard mínimo, sólo Ventas (sin Inventario / Financiero / Comercial).
  const MARI_ALLOWED_TABS: TabDash[] = ["ventas"];
      ...
      <header className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-1 max-w-md text-sm leading-relaxed" style={{ color: Z.muted }}>
              Nutriestéticans ERP · Vista {nivel === "supervisor" ? "de tu área" : "global"} · período alineado al filtro
            </p>
          </div>
        </div>
